...
BaseRect MaxRectsBinPack::Insert(int width, int height, int method)
    {
    BaseRect newNode;
...
void MaxRectsBinPack::Insert(std::vector<BaseRectSize> &rects, std::vector<BaseRect> &dst, FreeRectChoiceHeuristic method)
    {
    dst.clear();
    ...
    while(rects.size() > 0)
        ...
        for(int i = 0; i < rects.size(); ++i)
            ...
            int score1;
            int score2;
            BaseRect newNode = ScoreRect(rects[i].width, rects[i].height, method, score1, score2);
            newNode.name = rects[i].name;
...
void MaxRectsBinPack::PlaceRect(const BaseRect &node)
    {
    size_t numRectanglesToProcess = freeRectangles.size();
    for(size_t i = 0; i < numRectanglesToProcess; ++i)
        {
...
BaseRect MaxRectsBinPack::ScoreRect(int width, int height, FreeRectChoiceHeuristic method, int &score1, int &score2) const
    {
    BaseRect newNode;
...
BaseRect MaxRectsBinPack::FindPositionForNewNodeBottomLeft(int width, int height, int &bestY, int &bestX) const
    {
    BaseRect bestNode;
    memset(&bestNode, 0, sizeof(BaseRect));
...
BaseRect MaxRectsBinPack::FindPositionForNewNodeBestShortSideFit(int width, int height,
...
BaseRect MaxRectsBinPack::FindPositionForNewNodeBestLongSideFit(int width, int height,
        int &bestShortSideFit, int &bestLongSideFit) const
    {
    BaseRect bestNode;
    memset(&bestNode, 0, sizeof(BaseRect));
...
BaseRect MaxRectsBinPack::FindPositionForNewNodeBestAreaFit(int width, int height,
...
BaseRect MaxRectsBinPack::FindPositionForNewNodeContactPoint(int width, int height, int &bestContactScore) const
    {
    BaseRect bestNode;
    memset(&bestNode, 0, sizeof(BaseRect));
...
bool MaxRectsBinPack::SplitFreeNode(BaseRect freeNode, const BaseRect &usedNode)
    {
